...
from app.api.deps import CurrentUser, GenerateMediaFromTextRequestWithCost, SessionDep, GenerateMediaByMediaRequestWithCost
from app.models import (
    Media, MediaResponse, GenerateMediaFromTextResponse,
    GenerateMediaByMediaResponse, GetMediaByUserRequest, 
    GetMediaByUserResponse,
...
async def generate_media_from_text(
    request: GenerateMediaFromTextRequestWithCost,
    current_user: CurrentUser,
    session: SessionDep
):
    # Check if user has sufficient credits
    if not crud.has_sufficient_credits(session=session, user_id=current_user.id, required_credits=request.credit_cost):
        raise HTTPException(status_code=400, detail="Insufficient credits")

    # Enqueue Celery task
    task = generate_media_from_text_task.delay(request.model_dump(), current_user.id)

    return GenerateMediaFromTextResponse(
        task_id=str(task.id),
